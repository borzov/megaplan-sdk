...
async def test_expand_valid_fields(megaplan_api, deals):
    ...
    assert len(result) == 1
    assert result[0].deal.id == 1
    assert result[0].manager_details is not None
    assert result[0].manager_details.first_name == "John"
    assert result[0].contractor_details is not None
    assert result[0].contractor_details.name == "Test Corp"
...
async def test_expand_none_expand(megaplan_api, tasks):
    ...
    assert not hasattr(result[0], "responsible_details")
...
async def test_expand_list_entities(megaplan_api, tasks):
    ...
    assert result[0].responsible_details is not None
    assert result[0].responsible_details.first_name == "John"
    assert result[0].owner_details is not None
    assert result[0].owner_details.first_name == "Jane"
...
async def test_expand_entities_missing_field(megaplan_api, tasks):
    ...
    assert len(result) == 2
    assert result[0].responsible_details is None  # Task 1 has no responsible
    assert result[1].responsible_details is not None  # Task 2 has responsible
    assert result[1].responsible_details.first_name == "John"
