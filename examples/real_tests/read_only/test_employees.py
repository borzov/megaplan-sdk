...
async def test_iterate_employees():
    """Test iterating over employees."""
    print_header("TEST: Автоматическая пагинация сотрудников (iterate)")
    ...
    try:
        async with MegaplanClient(
            ...
            print("\n⏳ Итерация по сотрудникам (первые 15)...")
            count = 0
            async for employee in client.employees.iterate(limit=10):
                count += 1
                if count <= 5:
                    print(f"  {count}. [{employee.id}] {employee.display_name()}")
                if count >= 15:
                    break

            print_success(f"Обработано сотрудников: {count}")
            return True
...
async def test_list_with_pagination():
    ...
    print_header("TEST: Пагинация сотрудников")
    ...
    try:
        async with MegaplanClient(
            ...
            print("\n⏳ Загрузка первой страницы (5 сотрудников)...")
            first_page = await client.employees.list(limit=5)
            if not first_page:
                print_warning("Нет доступных сотрудников для тестирования")
                return False

            print_success(f"Загружено сотрудников на первой странице: {len(first_page)}")

            if len(first_page) >= 5:
                last_employee = first_page[-1]
                print(f"\n⏳ Загрузка второй страницы (после сотрудника #{last_employee.id})...")
                second_page = await client.employees.list(
                    limit=5,
                    page_after={"contentType": "Employee", "id": last_employee.id}
                )
                print_success(f"Загружено сотрудников на второй странице: {len(second_page)}")
...
async def run_all_tests():
    ...
    # Test 5: Iterate employees
    results.append(await test_iterate_employees())
