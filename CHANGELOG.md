...
- **Критично: `todos.update()` создавал дубликат дела вместо обновления.**
  ...
  не мутируется. `_update_entity()` в `resources/base.py` остаётся общим
  для `tasks`/`deals`/`todos` и не трогался: у `tasks`/`deals` этот путь на
  стенде проблем не показал, поэтому фикс точечный, только в
  `TodosResource.update()` — если аналогичный дефект найдётся у других
  ресурсов, чинить нужно будет отдельно.
