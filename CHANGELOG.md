...
- `megaplan_sdk.sync.TodoSync` (+ `TodoSyncState`, `TodoChanges`) —
  ...
  этой семантики — `examples/cookbook/todo-sync.md`. `TodoSyncState.cursor_id`
  вычисляется и сериализуется, но зарезервирован на будущее: `poll()` его не
  читает и не использует для сужения выборки — не полагайтесь на него.
  **Цена `poll()`:** серверного фильтра «что изменилось» у `Todo` нет, поэтому
  каждый вызов проходит весь список дел аккаунта целиком постранично по 100 —
  `window_days` на это число запросов не влияет, он ограничивает только объём
  сохраняемого состояния. Ориентир: 500 дел ≈ 6 запросов на `poll()`, 50 000 —
  ≈ 500; частоту опроса выбирайте по размеру аккаунта.
...
### 🐛 Исправлено
- **`TodoSync.poll()` ложно репортил `updated` на холостом повторе.**
  Подтверждено на боевом стенде: отпечаток дела включал поле `responsible`
  как есть, а сервер отдаёт его полным профилем `Employee` — с
  `isOnline`/`lastOnline`, которые меняются почти на каждый запрос, пока
  ответственный сотрудник активен, и не имеют отношения к самому делу.
  Из-за этого `TodoSync.poll()` показывал дело как «изменённое» на втором
  подряд опросе без единого реального изменения. `_fingerprint()` теперь
  сводит ссылочные поля (`responsible`) к их `id` перед хешированием — это
  заодно убирает и шум от вложенной дедупликации ссылок (#BUG-4), и саму
  нестабильность presence-полей, одним и тем же способом.
- **Критично: `todos.update()` создавал дубликат дела вместо обновления.**
  Подтверждено на боевом стенде: `POST /api/v3/todo/{id}` **без** `"id"` в
  теле JSON игнорирует `{id}` из пути и молча создаёт новое дело (200 OK,
  `data.id` другой), оставляя исходное нетронутым — каждый вызов
  `update()` плодил в проде сиротское дело. `TodosResource.update()` теперь
  сам кладёт `"id"` в тело (переопределяя любой `id`, случайно переданный
  вызывающим кодом) — вызывающему коду ничего менять не нужно, `todo_data`
  не мутируется. **Дефект специфичен для `Todo` на сервере, а не для формы
  запроса из общего `_update_entity()`**: `tasks.update()`, `deals.update()`
  и `projects.update()` отдельно проверены на живом API (round-trip чтение +
  обход списка на лишние сущности) и дубликатов не создают, 3 из 3 —
  пользователям 0.5.x/0.6.0 на этих трёх ресурсах бояться нечего. Фикс
  точечный, только в `TodosResource.update()`; `contractors.update()` и
  `employees.update()` таким же образом на этом аккаунте не проверялись
  (создать/удалить тестового контрагента нечем — `POST /api/v3/contractor`
  405-ит и `delete()` у ресурса нет, а сотрудников трогать нельзя) —
  остаются непроверенной гипотезой, не подтверждённым багом.
  **Серверная особенность шире одного метода SDK**: любой код — не только
  этот SDK — который ходит в `POST /api/v3/todo/{id}` без `"id"` в теле
  JSON, будет плодить дела-сироты на живом аккаунте (сервер молча создаёт
  новую запись вместо обновления существующей). Это полезно знать при
  отладке несвязанных с SDK интеграций и скриптов, использующих Todo API
  напрямую, а не только как объяснение самого фикса.
- **`todos.busy_days()` слал несуществующие параметры `from`/`to`** —
  RAML для `/todo/busyDays` (`docs/megaplan.raml`) их не знает вообще,
  сервер отвечал 422 «There is extra field "from"...» на каждый вызов.
  Метод больше не принимает `from_date`/`to_date` — только опциональный
  `filter` (`TodoFilter`), как и остальные `list`-подобные методы; вызов
...
- **`todos.list(sort_by=...)` с ключом `field` вместо `fieldName` 422-ил**
  ...
  в SDK. `TodosResource.list()` теперь сам бросает понятный `ValueError`
  при неверном ключе — вместо непрозрачного 422 от сервера.
- **`todos.list(q=...)` — сырой `q` сервер молча игнорировал** (тот же
  класс проблемы, что #11 у `tasks`/`deals`: 50 строк с `q` == 50 строк
  без `q`, ни одна не содержала фрагмент). `q` теперь конвертируется в
  реальный фильтр по `name` через `TodoFilter`, как и у `tasks`/`deals`;
  подтверждено на стенде. Передача одновременно `q` и `filter` теперь
  явно запрещена (`ValueError`), `q_in` с неподдерживаемым полем — как и у
  `tasks`/`deals` — бросает `NotImplementedError`.
...
- **`attachments.upload()` падал сырым `IndexError`, если сервер отвечал
  200 с пустым `data`.** Теперь в этом случае бросается `MegaplanError` с
  понятным сообщением вместо необъяснимого `IndexError` из недр SDK.
...
- Поле `history` у `DealFullDetails`, `TaskFullDetails` и `ProjectFullDetails`
  ...
  Changeset)`). **У `get_full_details(include_history=True)` нет параметра
  `raw=`, в отличие от `get_history()`, и отката к прежнему поведению нет.**
  На 0.6.0 этот вызов у `deals`/`tasks`/`projects` отдавал `list[dict]` —
  начиная с 0.6.1 это ломающее изменение без обходного пути внутри самого
  вызова. Если вашему коду нужен именно сырой журнал — вызывайте отдельно
  `get_history(entity_id, raw=True)` вместо `get_full_details(...).history`.
...
### Документация
- Порядок записей журнала (`get_history()`/`iterate_history()`/
  `get_link_events()`) приведён к единой формулировке — **сначала новые**.
  Докстринги пяти публичных фасадов (`deals`/`tasks`/`projects`/
  `contractors`/`todos`) это и так утверждали; расходился только внутренний
  `_get_link_events` в `_history.py` («oldest page first») — он был просто
  неверен, исправлено. Подтверждено эмпирически на стенде 2026-08-21:
  журнал реальной сделки, 6 записей, `timeCreated` монотонно убывает. Это
  наблюдаемый дефолт сервера (`sort_by` в запрос не передаётся), а не
  задокументированный контракт API — код, которому важен порядок, не должен
  индексировать `events[0]`/`events[-1]`, а использовать `since_id`/
  `since_time` (кукбук `link-tracking.md` теперь явно предупреждает об этом).
- `README.md` больше не утверждает, что загрузка вложений «не реализована»
  — `attachments.upload()` описана с примером (загрузка → прикрепление к
...
- **Где мы стоим на RAML-only маршрутах, взятых на веру:** на 0.6.1 живьём
  ...
  (комментарий, не поле), `status_id` подтверждён неявно (сервер сам выбрал
  финальный статус), а `result_attaches`/`notify_contractors` остаются
