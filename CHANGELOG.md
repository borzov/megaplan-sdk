...
- `todos.search_history()` и `contractors.search_history()` — у сделок, задач
  и проектов поиск по журналу уже был, теперь есть и у этих двух ресурсов;
  у `contractors` он, как и остальные методы журнала, идёт через конкретный
  подтип (`ContractorCompany`/`ContractorHuman`) — абстрактный
  `/contractor/{id}` для этого 500-ит.
- `todos.get(fields=...)` — запрос дополнительных полей карточки дела
  (например, `["commentsCount"]`), как у `tasks.get()`/`deals.get()`.
- `todos.list()`: параметры `page_before`, `page_with`,
  `only_requested_fields` и `expand` (`responsible`, `user_created`) —
  раньше был только `page_after`, список отставал от остальных ресурсов.
- `page_after` у `get_todos()` пяти ресурсов (`deals`, `tasks`, `projects`,
  `employees`, `contractors`) — постраничная выгрузка дел сущности,
  которой раньше не хватало, хотя нижележащий миксин её уже умел.
...
- `AuthResource` использовал собственный независимый `AuthManager` вместо
  клиентского. Из-за этого `client.auth.get_access_token()` всегда возвращал
  `None`, а `client.auth.clear_tokens()` не затрагивал токен, которым
  реально пользуется транспорт, — «очищенный» клиент мог на следующем 401
  незаметно воскресить всё ещё живой refresh-токен. Теперь `client.auth`
  разделяет состояние токена с клиентом: `get_access_token()` возвращает
  настоящий токен, а `clear_tokens()` по-настоящему очищает провайдера
  токенов транспорта.
- `todos.list(q_in=[...])` пропускал поле `statement`, которого у `Todo` нет:
  сервер отвечал 200 с полной, нефильтрованной выборкой вместо ошибки —
  тот же класс ловушки, что и у `tasks`/`deals` (см. #11 в предыдущих
  релизах). Список разрешённых полей для `Todo` теперь ограничен `name`.

### Изменено

- `attachments.upload()` читает файл в отдельном потоке
  (`asyncio.to_thread`) вместо блокирующего чтения на событийном цикле —
  раньше multipart-энкодер httpx вычитывал файловый дескриптор синхронно
  прямо внутри `async`-метода. Заодно это делает автоповтор запроса при
  обновлении токена безопасным: в транспорт уходят уже прочитанные байты,
  а не дескриптор, который повторное чтение не найдёт открытым.
