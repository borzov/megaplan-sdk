...
Отдельно ничего вызывать не нужно — кроме `stream_binary()` (скачивание
вложений): там обновление работает только проактивно, повтор после 401 на
уже открытом стриме не выполняется.

Срок действия токена не сохраняется между запусками: конструктор
`MegaplanClient` принимает `access_token`/`refresh_token`, но не их срок
действия, поэтому после перезапуска процесса SDK не знает, когда токен
истекает, и до первого 401 работает только реактивная половина
автообновления — первый запрос платит за это одним лишним обращением к серверу.
...
async with MegaplanClient(
    ...
    # Первый запуск: tokens.json ещё нет, saved пуст — без пароля клиент
    # останется без единого токена и первый же запрос упадёт с 401.
    username=None if saved else "user@example.com",
    password=None if saved else "password",
...
прерывает запрос. При ручном вызове `authenticate()`/`refresh_token()` это не
страшно — токен-пара в любом случае возвращается методом вызывающему коду. Но
при автоматическом обновлении внутри `_request()` (проактивном или по 401)
это невосполнимая потеря: ротированная пара приложению никак не передаётся, а
прежний `refresh_token` к этому моменту уже мёртв на сервере — после
перезапуска процесса восстановить сессию будет нечем. Поэтому сохранение
токенов в колбэке должно быть надёжным и не должно падать молча.
